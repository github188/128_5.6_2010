...
namespace KJ128A.Ports
{
    ...
    /// 
    /// </summary>
    public class KJTcpClientPort : KJ128A_TCP
    {
        ...
        /// 主机故障时备机巡检周期时间（毫秒）
        /// </summary>
        private const int intHostTime = 1200;

        /// <summary>
        /// 主机正常时备机巡检周期时间（毫秒）
        /// </summary>
        private const int intBackerTime = 1900;
        /// <summary>
        /// 备机发送次数
        /// </summary>
        ...
        public KJTcpClientPort(ArrayList socketArrayList, int iGroup, int iMark)
        ...
        public delegate void DataReceivedEventHandler(byte[] cmdReceived, int iMark);
        ...
        public void SendCmd()
        {
            //清空   重启Timer的计时器 qyz 2012-11-30
            if (sendAgainTime != null && sendAgainTime.Enabled)
            {
                sendAgainTime.Enabled = false;
                sendAgainTime.Start();
            }
                                ...
                                if (!_IsHost)
                                {
                                    ...
                                    if (_Stations[iStationLoc].State != -10000)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                StationStateChanged(1, _Stations[iStationLoc].Address, _Stations[iStationLoc].State, _Stations[iStationLoc].CState);
                                            }
        ...
        private void AnalysisPolling_20071210_1(byte[] cmdRight)
        {
            ...
            while (iCmdLoc < iCmdLength + 5)
            {
                ...
                StringBuilder sbHeadE = new StringBuilder();    // 探头 接收到全为 1 的发码器   即求救
