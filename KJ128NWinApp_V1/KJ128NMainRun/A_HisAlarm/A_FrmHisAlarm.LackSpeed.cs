...
namespace KJ128NMainRun.A_HisAlarm
{
    partial class A_FrmHisAlarm
    {
        ...
        private string StrWhere_LackSpeed()
        {
            string strTempWhere = " 起始读卡分站监测时间 >= '" + dtp_LackSpeed_Begin.Text +
                "' And 起始读卡分站监测时间 <='" + dtp_LackSpeed_End.Text + "' ";
        ...
        public void SelectInfo_LackSpeed(int pIndex)
        {
            if (pIndex < 1)
            {
            ...
            DataSet ds = rtabll.GetInfo_HisLackSpeed(pIndex, pSize, strWhere_LackSpeed);
            ...
            if (ds.Tables != null && ds.Tables.Count > 1)
            {
                ...
                countPage = sumPage;
                ds.Tables[0].TableName = "A_FrmHisAlarm_LackSpeed";
                if (sumPage == 0)
                {
